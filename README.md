...
方向键选择要写入的目标硬盘（本例 `nvme0n1`，仅一个目标盘，直接回车`Enter`即可），回车`Enter`确定。该盘现有资料会被覆盖。初学模式会按源盘分区表原样还原（本例是 1G EFI + 1.8T 系统分区），因此目标盘容量需 ≥ 源盘，不是看镜像文件有多大，也不是看已用空间。本例源盘和目标盘都是 2000GB 同型号；换到更小的盘或不同型号时，可能还原失败或影响引导，本文不覆盖缩小分区的做法。确认型号和容量无误后再继续，如图 37 所示。
